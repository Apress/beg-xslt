...
<xsl:key name="programsByHour" match="Program" 
         use="substring(Start, 12, 2)" />
<xsl:key name="programsByHourAndMinute" match="Program"
         use="substring(Start, 12, 5)" />
...
<xsl:template match="TVGuide">
  <xsl:apply-templates 
    select="Channel/Program
             [generate-id() = generate-id(key('programsByHour', 
                                              substring(Start, 12, 2))[1])]" />
  <h2>Series</h2>
  <xsl:apply-templates select="Channel/Program/Series" mode="EpisodeList">
    <xsl:sort select="." />
  </xsl:apply-templates>
</xsl:template>
...
<xsl:template match="Program/Series" mode="EpisodeList">
  <div>
    ...
    <h4>Episodes</h4>
    <ul>
      <xsl:for-each select="key('programsBySeries', .)">
        <li>
          <a href="#{generate-id()}">
...
<xsl:template match="Program">
  ...
  <div class="hour">
    <h2>Showing from <xsl:value-of select="$hour" />:00</h2>
    <xsl:for-each 
      select="key('programsByHour', $hour)
                [generate-id() =
                 generate-id(key('programsByHourAndMinute',
                                 substring(Start, 12, 5))[1])]">
      <xsl:sort select="substring(Start, 12, 5)" />
      <xsl:variable name="time" select="substring(Start, 12, 5)" />
      <div class="minute">
        <h3><xsl:value-of select="$time" /></h3>
        <xsl:for-each select="key('programsByHourAndMinute', $time)">
          <div>
