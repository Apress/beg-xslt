...
<xsl:param name="series" select="'StarTrek'" />
...
<xsl:template match="TVGuide">
  ...
  <xsl:choose>
    <xsl:when test="$Channels">
      ...
      <xsl:variable name="Plural" select="$NumberOfEpisodes > 1" />
      <p>
        There
        <xsl:choose>
        ...
        <xsl:value-of select="concat($NumberOfEpisodes, ' ', $series)" />
        episode<xsl:if test="$Plural">s</xsl:if>
        showing this week.
      </p>
